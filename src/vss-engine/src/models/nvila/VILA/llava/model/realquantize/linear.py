...
def fp8matmul(a, b, output_quantize, scale_a, scale_b, QB, stochastic=False):
...
def fp8_linear_forward(x, s, w, s_w, output_quantize, QB):
    w_t = w.t()
    return fp8matmul(x, w_t, output_quantize, s, s_w, QB)
...
def fp8_linear_backward(
    x_t, s, g, s_g, g_t, w_t, s_w, QB, stochastic=False, dgrad_quantize=True
